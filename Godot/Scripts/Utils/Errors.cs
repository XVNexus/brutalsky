...
public static class Errors
{
    public static ArgumentOutOfRangeException InvalidItem(string type, object value)
        => new(nameof(value), $"'{value}' is not a valid {type}");

    public static ArgumentOutOfRangeException NoItemFound(string type, object id)
        => new(nameof(id), $"No {type} found with id '{id}'");

    public static InvalidOperationException RegisterActivePlayer(string id)
        => new($"Player '{id}' is already active and cannot be registered again");

    public static InvalidOperationException UnregisterInactivePlayer(string id)
        => new($"Player '{id}' is already inactive and cannot be unregistered again");
    ...
    public static ArgumentOutOfRangeException OversizedMap(Vector2 size, float limit)
        => new(nameof(size), $"Map size {size} exceeds the limit of {limit}");
